<template>
  <v-container >
    <v-data-table :headers="headers" :items="trackings" :search="search" class="elevation-1">
      <template v-slot:top >
                ...
                <v-container>
                  <v-row>
                    <v-col cols="12" sm="6" md="4">
                      <v-text-field v-model="form.tcode" label="Tracking code*"></v-text-field>
                    </v-col>
                    <v-col cols="12" sm="6" md="4">
                      <v-text-field v-model="form.descript" label="Descrição*"></v-text-field>
                    </v-col>
                  </v-row>
...
import Tracking from "@/services/trackings";
export default {
  components: { Layout },
  data: () => ({
    search: "",
  ...
  methods: {
    ...
    deletar(item) {
      ...
      confirm("Tem certeza que quer deletar o código?") &&
  ...
  mounted() {
      Tracking.listar().then(resposta => {

          console.log(resposta.data)
          this.trackings = resposta.data
